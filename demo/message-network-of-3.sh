...
launch_bg "./$BIN --server-addr=\"127.0.0.1:4002\" --peer-id=2 --dht-filename=/tmp/p2p-demo/b.dht seed" "/dev/null"
server_pid2=$!
sleep 1

section "Bootstrap peer C (from peer B)"
launch "./$BIN --server-addr=\"127.0.0.1:4003\" --peer-id=3 --dht-filename=/tmp/p2p-demo/c.dht bootstrap 127.0.0.1:4002"

section "Now, we have this network C -> B -> A, let's send a message from C to A"
launch "./$BIN --server-addr=\"127.0.0.1:4003\" --peer-id=3 --dht-filename=/tmp/p2p-demo/c.dht message 1 \"Hello A, this is C speaking!\""
