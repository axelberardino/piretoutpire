...
// For demonstration purpose 64 Ko is handy.
// For real world files, 1 Mo is better.
pub const DEFAULT_CHUNK_SIZE: u32 = 64 * 1024; // 64 Ko chunks.

pub type FileChunk = FileFixedSizedChunk<DEFAULT_CHUNK_SIZE>;

pub struct FileFixedSizedChunk<const CHUNK_SIZE: u32> {
    file_size: u32,
    writer: BufWriter<File>,
    reader: BufReader<File>,
}

impl<const CHUNK_SIZE: u32> FileFixedSizedChunk<CHUNK_SIZE> {
        ...
        P: AsRef<Path>,
    {
        ...
        Ok(Self {
            file_size: file.metadata().context("can't get file size")?.len() as u32,
    ...
    pub fn open_new<P>(path: P, preallocated_size: u32) -> AnyResult<Self>
    where
        P: AsRef<Path>,
    {
    ...
    pub fn read_chunk(&mut self, chunk_id: u32) -> AnyResult<Vec<u8>> {
        read_range(
            &mut self.reader,
            chunk_id * CHUNK_SIZE,
    ...
    pub fn write_chunk(&mut self, chunk_id: u32, data: &[u8]) -> AnyResult<()> {
        write_range(
            &mut self.writer,
            chunk_id * CHUNK_SIZE,
            min((chunk_id + 1) * CHUNK_SIZE, self.file_size),
...
fn read_range<T>(br: &mut T, from: u32, to: u32) -> AnyResult<Vec<u8>>
where
    T: BufRead + Seek,
{
    if from > to {
...
fn write_range<T>(bw: &mut T, from: u32, to: u32, data: &[u8]) -> AnyResult<()>
where
    T: Write + Seek,
{
    if from > to {
